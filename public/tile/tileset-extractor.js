function onLoad() {
  ...
  function extract(src) {
    ...
    source.onload = function() {
      ...
      if (checkSourceSize()) beginExtractionWorker();
    };
  ...
  function beginExtractionWorker() {
    ...
    worker.onmessage = function(event) {
      ...
      if (action == "extract-start") {
        ...
        downloadMapLink.href = window.URL.createObjectURL(
          new Blob(
            [
              JSON.stringify({
                map: map,
        ...
        downloadTiledTMXLink.href = window.URL.createObjectURL(
          new Blob([exportTiledFormat()], { type: "text/xml" })
        );
      }
    };
